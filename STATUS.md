...
最終更新: 2026-07-29 第 8 便(branch `fable5-mainline`)
状態: **第 8 便で Gate 7 の一様減衰を定理として説明し、探索目的を臨界ノルムの
初期生成率に置き換えた。** 古典的な `L^p` エネルギー恒等式(p=3)から
`J(u_0) = d/dt||u||_3^3|_{t=0} = 3∫p div(|u|u) - 3ν∫(|u||∇u|^2 + |u||∇|u||^2)`
を導出。移流項は**全ての発散ゼロ場で厳密にゼロ**、粘性項は非正。したがって
正の生成率は**圧力項からしか出ない**。純粋旋回では圧力が軸対称なので
`u_0·∇p = u^θ r^{-1} ∂_θ p = 0` が各点で成立し、`J(u_0) ≤ 0`(等号は `u_0 ≡ 0`
のみ)。族 S/A/H は全て純粋旋回なので、Gate 7 の 18 run 全ての減衰は測定結果では
なく定理の帰結だった。**これは探索レーンの棄却であって、何かの支持ではない。**
混合初期値族 M1/M2/M3 を構成し(swirl + meridional、発散ゼロは代数的恒等式、
全偏微分は解析的)、**軸パリティ選択則**を導出: `ψ_1` が z について奇でない限り
圧力項は z 対称領域上で恒等的に消える(swirl のパリティは無関係)。スケール不変な
目的関数 `Re_crit = A L (|V|/ν) / P` を定義し、8 振幅の固定基底上で multi-start
最適化。**`J > 0` は達成した**(最適形状は `Re > 約 1.4e3` で正、収束確認済み)が、
**事前登録したキル条件(`Re_crit < 1e2`)は発火**し、形状因子レーンは棄却された。
**厳密区間証明書は `J > 0` を証明できなかった**(包絡が 60 桁緩い)ので、
規則どおり候補は昇格させていない。時空スラブ証明書の **H1 を Bernstein 凸包で
定理に格上げ**(仮説から証明へ)、**H2 は枠組み自体が不健全と判明したため撤回**。
Lean は L3 生成恒等式の代数核と control ODE 比較補題を追加し、全 75 定理が
古典公理のみ。**Clay 問題を解決したという主張は一切していない。**

前便までの状態: 第 7 便で Picard 領域を離れ τ=1 に到達、乖離ゲート 9 項目合格、
昇格候補ゼロ、Lean で F-7c を直接経路で閉じた。第 3-6 便で固定有限帯域 no-go、
線形 Gate 4、Green 微分 tail 上界、中振幅 Gate 6 を実施。
