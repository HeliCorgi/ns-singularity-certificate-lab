...
最終更新: 2026-07-29 第 7 便(branch `fable5-mainline`)
状態: **第 7 便で Picard 領域を離れた。** 無次元化を完成させ、系の唯一の
パラメータが `Re = A L²/ν` であることを確定。第 6 便の 32 点スイープを
`(Re, aspect, c, τ)` 座標へ再分類したところ、**到達していた無次元時間は最大でも
`τ = 0.0233`**(公称振幅が実振幅を約 23 倍過大評価していたため)であり、
全 run が第一 Picard 反復に留まっていた理由が定量的に判明した。Picard 梯子
(level 0/1/2 + 完全解を同一積分器・同一 step で同時積分)を実装し、乖離を推定
ではなく実測に変えた。前登録した τ = {0.025 … 1.0} と Re = {10 … 400} × 族 S/A/H
の 18 run をすべて τ=1 まで実行。**第一 Picard 乖離ゲートは全 9 項目合格**
(乖離 36 %、adv/src 0.49、積分器一致 4.4e-9、dt 半減一致 8.5e-12、前線 10/11 点)。
**昇格候補はゼロ**: 18 run すべてで臨界 `L³` ノルムが単調減少し、dyadic shell 数は
16 run で増加(集中ではなく拡散)。最良の A_Re400 でも 6 条件中 1 条件のみ充足。
Lean は **F-7c を直接経路で閉じた**(mathlib の `IsPicardLindelof` は元から時間
依存であり、自励化は不要だった)。時空スラブ証明書(cell 内部・全時刻を包含、
独立 checker + 改竄拒否 4 種)と `H^s` 誤差伝播の**不完全な**導出(必要補題
HS-1…HS-6 を明示、`L^∞` 証明書を無条件 PDE 証明として提示しない旨を明記)を追加。
**Clay 問題を解決したという主張は一切していない。**

前便までの状態: FABLE5_NEXT_TASK_AUDIT の P0 ゲート実装・実行済み。第 3 便で
Track F の**固定有限次元・固定帯域** ansatz 族を除外定理として閉じ Lean(F-6)で
証明。第 4 便で Track U の線形 Gate 4 に合格、帯域幅発散候補の実現可能指数領域
(`0<γ<1` の三角形。現在の shell ansatz と非退化仮定の下で `γ≥1` は排除)を導出。
第 5 便で Gate 5 合格: Green 核の解析微分による微分 tail 上界、自由空間速度回復
API(2 独立経路)、小振幅全空間非線形 run、Lean F-14/F-15/F-16。第 6 便で非線形
tail 伝播の明示上界、明示的全空間初期値族、動的領域拡大、単一 snapshot の区間
証明書、Lean F-17/F-18/F-19 を追加。Gate 6 は 2 つの前登録基準に不合格で候補ゼロ。
