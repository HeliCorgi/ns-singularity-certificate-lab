...
> **Proposition 3: `γ ≥ 1` は全面的に排除される。** `σ > 2γ−1` と `σ < γ` が
> 両立するには `2γ−1 < γ`、すなわち `γ < 1`。
...
任意の多項式より速く消える。**帯域幅発散候補において、外力は Track U に対する
優位性を一切与えない。** 外力ができるのは `O(1)` シェルの整形だけである。
...
0. 〔第 4 便で更新〕**全空間の解像度設計**が Track U の新しい律速である。
   線形 Gate 4 は合格したので楕円側の障害はなくなった。次は
   `core_width.fit_precondition`(front ≥ 7 点)を全空間 box で評価し、
   適応 mesh か半周期 sine 実装かを決める。**この判断の前に非線形全空間実行を
   開始しない。** 次いで **F-7c**(時間依存外力での ODE 延長)を
   `docs/final_target.md` §4.1 の 2 経路のどちらかで閉じる。
