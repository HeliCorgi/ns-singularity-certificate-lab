...
最終更新: 2026-07-30 第 9 便(branch `fable5-mainline`)
状態: **第 9 便で周期 T³ の厳密 a posteriori レーン(Track P)を新設し、最初の
12 スラブ証明書がすべて閉じた。** 有理係数・平均ゼロ・発散ゼロの有限 Fourier
初期値 3 族(P1 helical triad / P2 連結 2 triad / P3 対称性破れ・圧力駆動 —
有限三角多項式なので C^∞、Lean 証明つき)に対し、厳密有理数演算の Fourier
機構(積和公式による convolution — FFT なし・エイリアシングなし、per-mode
Leray、H^n ノルム、time-Taylor 係数)を実装。Galerkin 軌道の**連続** NS 残差
`(I−P_G)P(u·∇u)` は有限三角多項式として厳密に計算され、**HS-5 の周期版は
構成的に閉じた**(全空間版は未解決のまま; 両者の混同は禁止)。自前導出の
H⁴ control 不等式 `d⁺R ≤ (−ν+9(K₁+K₂))R + 135Ȧ R² + ‖e‖_Ḣ⁴`(必要な埋め込みは
格子定数 Ȧ 一つ; n=3 で閉じるには MP の鋭い Kato 定数が要る — 記録済みの
トレードオフ)+ Picard 自己写像による軌道のスラブ包含 + control ODE 管により、
**3 族 × 4 スラブ長の 12/12 で「真の周期強解がスラブ全体に存在し
‖u−u_a‖_Ḣ⁴ ≤ R(t)(相対 4.4e-4〜9.0e-4)」を証明**(古典外部定理
EXT-P1/P2/P3 の条件付き; 忠実記録、Lean 公理化なし、checker が公理化主張を
拒否)。固定帯域**軌道**(no-go の除外対象)と有限帯域**初期値**(未除外;
Track P の作業領域)の区別を文書と Lean(`TrackPFourier.lean` の反例つき)で
確定。R³ 側は候補基底を flat bump から **Gaussian–Hermite**(多項式×Gaussian、
微分閉包は Lean 定理)へ変更し、区間包絡の 60 桁破綻を解消(粘性下界は真値の
7 倍以内)。J 最適化を新基底へ移植: Re_crit = 316 (41×81, 4start×12iter) → 検証 331 (97×193) / 333 (129×257); flat-bump 最良 1410 の 4.2 倍改善だが kill 条件 1e2 は未達。
`J>0` の区間証明は再試行したが margin は依然負(圧力×流束積の包絡幅が残る
障害; 定量記録)。C^k スプラインを最終候補として扱わない guard
(`require_clay_admissible`)を実装。Gate 7 減衰の表現を修正: **定理が保証する
のは初期時刻の J ≤ 0 のみ**で、meridional 成分生成後の τ>0 での継続的減少は
数値観測。**特異点証明ではなく、Clay 問題も解決していない。**

前便まで: 第 8 便で L3 生成恒等式・純粋旋回 no-go・パリティ選択則・
Re_crit 目的関数、第 7 便で Picard 領域離脱と F-7c、第 3〜6 便で有限帯域
no-go・線形/中振幅 Gate・区間証明書層。
