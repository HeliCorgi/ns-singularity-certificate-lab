...
最終更新: 2026-07-29 第 4 便(Gate 4 合格、F-7a/F-7b/F-12/F-13 形式化、帯域幅発散候補の指数領域)
...
| U-3 | **O** | 候補自体が未発見。**線形 Gate 4 は 2026-07-29 第 4 便で合格**したが、非線形全空間発展は未着手 |
...
| F-α(残り = 帯域幅発散族) | **M(必要条件のみ)** | `docs/research_notes/track_f_shell_constraints.md`。実現可能領域は `0<γ<1`、`max(0,2γ−1) ≤ σ < γ`、`β>0` の三角形に限られ、**`γ ≥ 1` は全面排除**。候補は一つも構成していない |
| F-β | **O** | — |
| F-γ | **O** | — |
...
| F-N4 | 滑らかな外力の Fourier 係数は任意の多項式より速く減衰するので、**高シェルのエネルギー収支で外力は不可視**。カスケードは無外力の場合と同じ困難 | **M** |
...
3. Lean で閉じているのは F-2 / F-3 / F-6 / F-7a / F-7b / F-12 / F-13 の
   7 件と Clay 命題の定義のみ。**互いに接続されておらず**、
   `ClayStatement.lean` へも接続されていない。
4. したがって **(A)〜(D) のいずれについても、証明の骨格すら存在しない**。

## 8. 本書から導かれる次の最小の一手(第 4 便更新)

1. **全空間の解像度設計**(Track U、最優先): 線形 Gate 4 は通ったので、
   次の障害は「一様全空間格子で Hou 型 front を解像できるか」である。
