...
最終更新: 2026-07-29 第 5 便(Gate 5 合格: 微分 tail 上界・自由空間速度回復 API・小振幅全空間非線形 run、F-14/F-15/F-16/F-7c 還元、外力の役割分離と cascade 模型)
...
1. **強振幅での再ゲート**(Track U、最優先): Gate 5 は小振幅で通ったが、
   時間細分・領域拡大・境界次数の 3 つの部分ゲートは**この振幅では情報を
   持たなかった**(答えが桁まで一致)。場が外側境界に到達する振幅で
   3 つを再実行しなければ、外側境界条件は検証されていない。
   その後に「一様全空間格子で Hou 型 front を解像できるか」の判断へ進む。
