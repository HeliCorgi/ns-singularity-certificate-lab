...
| PO-05 | 離散化誤差 | コンピューター支援証明が必要 | manufactured convergence は前段検査。厳密上界なし |
...
| PO-14 | 独立実装による再現 | 数値的に検査可能 | manufactured identity の独立経路のみ。候補再現なし |
...
未知候補を探す前の最小の一手は PO-05/PO-14 の前段強化である。

1. manufactured candidate を物理 Cartesian 場へ復元する。
2. production の円柱微分を共有しない Cartesian finite-difference/complex-step spot checker で divergence と curl を再計算する。
3. 解像度系列から観測次数と tail proxy を保存する。
4. 診断改変を hash と再計算で拒否する。

これは PO-05 や PO-14 を閉じないが、将来の候補が循環的自己検証に依存するのを防ぐ、最小で再利用可能な前進である。
